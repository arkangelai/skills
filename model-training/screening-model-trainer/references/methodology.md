...
**Goal:** Hippocrates zip → CSV normalizado.

1. **`download_from_hippocrates`** del bucket `hippocrates`, ruta `<userId>/<projectId>/<file>.zip`.
2. **`load_arkformat_zip`** descomprime y normaliza al CSV de trabajo.
...
El pipeline de Hippocrates persiste `train.csv`/`val.csv`/`test.csv` + `background_shap_set.csv` junto al `.pkl` desplegado en `<bucket>/<userId>/<projectId>/<expId>/`.
...
**Decision rule:**
- TabPFN AUROC ≥ tuned GBM + 0.01 AND latencia <100ms/pred → **propose-N a Laura**.
...
Phase 3 ya requiere 1 baseline literario. Cuando existen múltiples (EPOC: PUMA, LFQ, SQ-COPD, COPD-PS, CDQ; CVD: Framingham, ASCVD, QRisk; renal: KFRE, MDRD, CKD-EPI), benchmarkear TODOS standalone antes de elegir cuál combinar en Phase 5.7.
...
Caso AZ-Oculus EPOC: PUMA ganó (AUROC 0.650, lowest corr con proba) → adoptado en 90/10 ensemble (Phase 5.7).
...
Si la baseline literaria de Phase 3 (KFRE, FINDRISC, PUMA, etc.) tiene discriminación comparable o complementaria sobre el test (dentro de 0.05 AUROC del modelo), evaluar Comfama-style weighted ensemble antes de lockear deploy.
...
**Patrones validados:**
- Comfama hipoglucemia: 60/40 model+score.
- AZ-Oculus EPOC: 90/10 model+PUMA (modesto +0.005 AUROC, +0.029 en F+non-smoker, defensa regulatoria más fácil que meta-stacking).
...
**Pause-point 🔴 si N_pos<200** — isotónica puede sobreajustar; sigmoid (Platt) es más estable. Confirmar con Laura.
...
Operating points son abstractos. Equipo clínico entiende "EPOC perdidos por mil tamizados" o "espirometrías innecesarias por mil"; no entiende "Spec 50.5% vs 42.2%". Toda comparación entre bundles o threshold candidatos DEBE incluir esta traducción.
...
- "Bajar Sens 85→80 cuesta ~30 EPOC perdidos / 1000 tamizados, ahorra ~12 espirometrías / 1000."
- Comparar cost ratio (1 missed case vs 1 unnecessary spirometry) a literatura clínica.

Para condiciones subdiagnosticadas (EPOC LATAM ~60-70%, CKD ~80-90%), costo de FN domina y "save spirometries" raramente justifica.
...
## Phase 11 — Hippocrates packaging (deferred)
...
**11-slide template** validado en AZ-Oculus 2026-05-05:
