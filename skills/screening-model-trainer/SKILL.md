...
El workflow son **13 fases principales (Phase 0–12) con sub-numeración linear N.X** (e.g., 1.1, 1.2, 1.3, 4.1, 4.2, 4.3, 5.1, 5.2, 5.3, 6.1, 6.2, 7.1, 8.1) mapeadas a 9 deliverables markdown TRIPOD+AI en `templates/`. Cada fase produce artefactos concretos y se registra en `docs/05_modeling_log.md`. Detalle completo en `references/methodology.md`.
...
6. **Phases 4.1, 4.2, 4.3 — Alternative architectures + multi-score benchmark (mandatory).** Tres sub-fases independientes:
...
14. **Phase 7 — Validation: LOIO + subgroups + DCA.** Leave-One-Institution-Out, subgroup tables (sex, age_bin, etc.), Decision Curve Analysis. **Phase 7.1 — Nested CV (opcional)** cuando `n<2000` y bootstrap CI half-width >0.05.
