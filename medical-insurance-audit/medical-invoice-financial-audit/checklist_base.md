...
Documento maestro del instrumento **FIN-CTR** (`checklist_base.json`). Explica cómo el sub-agente financiero llena el JSON, cómo orquesta `planes/` + `tarifarios/` + el contrato marco, y cómo documenta los controles antifraude.
...
|---|---|---|
| ¿Cuál es el plan del afiliado? | `BDUA` (externo) → `planes/INDEX.md` → `planes/plan_<id>.md` | F04, F06, F20, F21, F22, F25 |
| ¿Qué tarifario aplica? | Contrato → `tarifarios/INDEX.md` → `tarifarios/<nombre>.csv` | F07, F08, F09, F13 |
...
```
1. Tarifario contratado con la IPS (tarifario_contrato_eps_2026.csv)  ← siempre primero
...
- `"Plan ORO (planes/plan_oro.md §Topes anuales): ayudas diagnósticas sin tope. Servicio facturado dentro de cobertura."`
